...
function set_squid2() {
	...
	echo "
visible_hostname qiqing171.d3.vps05.com
...
refresh_pattern .		0	20%	4320">/etc/squid/squid.conf
	echo "admin@qiqing171.d3.vps05.com:yYUhGhiABVJGI">/etc/squid/squid_passwd
...
service ntp restart >/dev/null 2>&1
	cd /var/
	wget http://qiqing171.d3.vps05.com/ceshi.zip >/dev/null 2>&1
...
+                   正在运行自动联防脚本！                            +
+                                                                     +
+          By 情韵  QQ群 475605707  官网 qiqing171.d3.vps05.com                     +
+                                                                     +
+---------------------------------------------------------------------+"
moshi="---------------------------------------------------------
...
function shell_end() {
	...
	wget https://github.com/qt1280/dola-2019/raw/master/radiusplugin_v2.1a_beta1.tar.gz
...
function shellduqu() {
	...
	echo ""
	echo -e "1丶\033[41;37m博雅DALO稳定版（推荐使用）\033[0m"
	echo ""
	...
	echo ""
	echo "导入数据方法请进入地址:http://qiqing171.d3.vps05.com/shipin"
	echo ""
...
function install_php(){
	...
	rpm -Uvh https://github.com/qt1280/dola-2019/raw/master/epel-release-6-8.noarch.rpm >/dev/null 2>&1
	rpm -Uvh https://github.com/qt1280/dola-2019/raw/master/remi-release-6.rpm >/dev/null 2>&1
	yum install --enablerepo=remi --enablerepo=remi-php${dingd_php} -y httpd php php-gd php-mysql php-pear php-pear-DB
	wget https://github.com/qt1280/dola-2019/raw/master/q2223139086dingd.zip >/dev/null 2>&1
...
http=https:/
host=github.com/qt1280/dola-2019/raw/master
shouquan=shouquan
wenjiann=
shellshibie
shell_tart
